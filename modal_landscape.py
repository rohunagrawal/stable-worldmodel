"""Modal script to generate reacher loss landscape data for gradient.py visualization.

Produces the same file format as testing/pusht.* for both the pretrained and
adversarial reacher world models:
  testing/reacher.pretrained/{A,B}.npy  Z.pth  markers.npy  alphas_path.npy  betas_path.npy
  testing/reacher.adv/{A,B}.npy         Z.pth  markers.npy  alphas_path.npy  betas_path.npy
...
Usage:
    modal run modal_landscape.py
    modal run modal_landscape.py --epoch-adv 3 --grid-size 50
"""

import modal

app = modal.App("stable-worldmodel-landscape")
...
def generate_landscape(
    ...
    """Generate the loss landscape for both reacher models and return data + plot bytes."""
    import os
    import shutil
    import numpy as np
    import torch
    ...
    dataset = swm.data.HDF5Dataset(
        "dmc/reacher_random",
        keys_to_cache=["action", "qpos", "qvel"],
    ...
    # Fit action scaler on individual 2D actions (same as eval_wm.py)
    ...
    # Pick one episode with sufficient length
    ep_lens = np.array(
    ...
    # Load the episode chunk [0, GOAL_OFFSET+1)
    ...
    def process_frame(chw_uint8_tensor):
        """(C, H, W) uint8 tensor → (3, 224, 224) float32."""
        return img_transform(tv_tensors.Image(chw_uint8_tensor))

    # ep_data['pixels']: (T, C, H, W) uint8 tensor from _load_slice
    obs_frame = ep_data["pixels"][0]           # (C, H, W)
    goal_frame = ep_data["pixels"][GOAL_OFFSET]  # (C, H, W)

    # Shape for solver info_dict: (B=1, T_hist=1, C, H, W)
    ...
    # ── GT action sequence ───────────────────────────────────────────────────────
    # 25 raw actions → normalize → reshape to solver format (horizon=5, action_dim_flat=10)
    ...
    # ── Load models ───────────────────────────────────────────────────────────────
    # load_pretrained appends "checkpoints" to cache_dir internally, so pass STABLEWM_HOME
    stablewm_home = Path(STABLEWM_HOME)
    ckpt_dir = stablewm_home / "checkpoints"

    model_base = swm.wm.utils.load_pretrained("lewm-reacher", cache_dir=stablewm_home)
    model_base = model_base.to(device).eval()
    model_base.requires_grad_(False)

    # Copy epoch-N weights so load_pretrained picks them up
    adv_src = ckpt_dir / "lewm-reacher-adv" / f"weights_epoch_{epoch_adv}.pt"
    adv_dst = ckpt_dir / "lewm-reacher-adv" / "weights.pt"
    if adv_src.exists():
        shutil.copy2(adv_src, adv_dst)
        print(f"Copied {adv_src.name} → weights.pt")
    model_adv = swm.wm.utils.load_pretrained("lewm-reacher-adv", cache_dir=stablewm_home)
    ...
    def run_gd(model, obs_pix_dev, goal_pix_dev):
        """Run GD planning for one env. Returns (a_plan, all_step_actions)."""
        ...
        # Info dict: (B=1, T_hist=1, ...) — solver will expand the sample dim
        info = {
            "pixels": obs_pix_dev,
            "goal": goal_pix_dev,
        ...
        a_plan = outputs["actions"].cpu()[0]  # (5, 10)
    ...
    # ── Shared 2-D basis ─────────────────────────────────────────────────────────
    a_gt_flat = a_gt.numpy().flatten()          # (50,)
    d_alpha = (a_base - a_gt).numpy().flatten()  # GT → base plan
    d_beta = (a_adv - a_gt).numpy().flatten()    # GT → adv plan
    basis = np.stack([d_alpha, d_beta], axis=1)  # (50, 2)

    # ── Build grid ────────────────────────────────────────────────────────────────
    # Use ±1.5 to ensure GD paths (which can overshoot the plan at ±1) stay in frame
    alphas = np.linspace(-1.5, 1.5, grid_size)
    betas = np.linspace(-1.5, 1.5, grid_size)
    A_grid, B_grid = np.meshgrid(alphas, betas)  # (grid_size, grid_size) each
    ...
    def eval_grid_costs(model, obs_pix_dev, goal_pix_dev):
        """Evaluate model cost on all grid points. Returns (grid_size, grid_size) numpy array."""
        with torch.no_grad():
            goal_emb = model.encode({"pixels": goal_pix_dev})["emb"]  # (1, 1, D)
            obs_emb = model.encode({"pixels": obs_pix_dev})["emb"]    # (1, 1, D)

        costs_all = []
        for start in range(0, N, batch_size_grid):
            bs_actions = a_grid[start : start + batch_size_grid]  # (bs, 5, 10)
            bs = bs_actions.shape[0]

            # action_candidates: (bs, S=1, horizon, action_dim_flat)
            action_cands = bs_actions.unsqueeze(1)

            # info for rollout — pre-set emb to skip encoding
            # rollout expects: pixels (B, S, T, C, H, W), emb (B, S, T, D), goal_emb (B, T, D)
            # obs_pix_dev: (1, T=1, C, H, W) → unsqueeze S dim → (1, S=1, T=1, C, H, W)
            ...
            with torch.no_grad():
                ...
                cost = model.criterion(info)  # (bs, 1)
    ...
    def project_path(all_step_actions):
        """Project each GD step action into (α, β) coordinates."""
        alphas_p, betas_p = [], []
        for step_acts in all_step_actions:
            # step_acts: (1, 1, horizon, action_dim_flat)
    ...
    # Initial (zero-action) projection
    diff_init = -a_gt_flat
    ...
    Z_max_global = max(Z_base.max(), Z_adv.max())
    # Cap at 95th percentile for both norm and 3D so extreme spikes don't wash out structure
    ...
    plot_specs = [
        ("Pretrained WM", Z_base, alphas_path_base, betas_path_base,
         models_data["reacher.pretrained"]["markers"]),
        ("Adversarial WM", Z_adv, alphas_path_adv, betas_path_adv,
         models_data["reacher.adv"]["markers"]),
    ]
    ...
    for ax, (name, Z, ap, bp, markers) in zip(axes[2:], plot_specs):
        # Clip Z to vis_max so the surface shape is visible (extreme spikes flatten everything)
    ...
    plot_path = testing_dir / "reacher_landscape.png"
    fig.savefig(str(plot_path), dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Plot saved to {plot_path}")

    # ── Bundle results as tar archive for download (avoids torch on local machine) ─
    import io, tarfile
...
def main(
    ...
    """Run generate_landscape on Modal and save results locally."""
    import io, tarfile
    from pathlib import Path

    print(
        f"Generating reacher loss landscape "
    ...
    if ret.get("plot_bytes"):
        local_plot = Path("eval_results/reacher_landscape.png")
    ...
    if ret.get("tar_bytes"):
        ...
        for model_name in ("reacher.pretrained", "reacher.adv"):
