"""Modal script to generate OGBenchCube loss landscape data for gradient.py visualization.

Produces the same file format as testing/pusht.* for both the pretrained and
adversarial cube world models:
  testing/cube.pretrained/{A,B}.npy  Z.pth  markers.npy  alphas_path.npy  betas_path.npy
  testing/cube.adv/{A,B}.npy         Z.pth  markers.npy  alphas_path.npy  betas_path.npy
...
Usage:
    modal run modal_landscape_cube.py
    modal run modal_landscape_cube.py --epoch-adv 3 --grid-size 50
"""

import modal

app = modal.App("stable-worldmodel-landscape-cube")
...
def generate_landscape(
    ...
    """Generate the loss landscape for both cube models and return data + plot bytes."""
    import os
    import shutil
    import numpy as np
    import torch
    ...
    HORIZON = 5
    ACTION_BLOCK = 5
    # OGBench cube ManipSpace has 5D raw actions (dx, dy, dz, d_yaw, gripper)
    # effective_act_dim = frameskip * raw_action_dim = 5 * 5 = 25
    SINGLE_ACTION_DIM = 5
    ACTION_DIM_FLAT = ACTION_BLOCK * SINGLE_ACTION_DIM  # = 25
    # goal_offset_steps for cube (from modal_plan.py ENV_CONFIGS["cube"])
    GOAL_OFFSET = 50
    ...
    # Fit action scaler on individual 2D actions (same as training)
    ...
    # Pick one episode with sufficient length (need GOAL_OFFSET + 1 frames)
    ep_lens = np.array(
        [int(np.max(step_idx_data[ep_idx_data == ep])) + 1 for ep in ep_indices]
    )
    valid_eps = ep_indices[ep_lens >= GOAL_OFFSET + 1]
    ...
    # ── GT action sequence ───────────────────────────────────────────────────────
    # Use first HORIZON*ACTION_BLOCK = 25 raw actions as the reference GT plan
    GT_STEPS = HORIZON * ACTION_BLOCK
    a_gt_raw = ep_data["action"][:GT_STEPS].numpy()       # (25, 2)
    ...
    model_base = swm.wm.utils.load_pretrained("lewm-cube", cache_dir=stablewm_home)
    model_base = model_base.to(device).eval()
    model_base.requires_grad_(False)

    # Find the adversarial cube checkpoint (may be named lewm-cube-adv or lewm-ogbcube-adv)
    adv_ckpt_name = None
    for candidate in ("lewm-cube-adv", "lewm-ogbcube-adv"):
        if (ckpt_dir / candidate).is_dir():
            adv_ckpt_name = candidate
            print(f"Found adv checkpoint: {candidate}")
            break
    if adv_ckpt_name is None:
        raise FileNotFoundError(
            f"No adversarial cube checkpoint found in {ckpt_dir}. "
            "Tried: lewm-cube-adv, lewm-ogbcube-adv"
        )

    # Copy epoch-N weights so load_pretrained picks them up
    adv_src = ckpt_dir / adv_ckpt_name / f"weights_epoch_{epoch_adv}.pt"
    adv_dst = ckpt_dir / adv_ckpt_name / "weights.pt"
    if adv_src.exists():
    ...
    model_adv = swm.wm.utils.load_pretrained(adv_ckpt_name, cache_dir=stablewm_home)
    ...
    def run_gd(model, obs_pix_dev, goal_pix_dev):
        ...
        info = {
            "pixels": obs_pix_dev,
            "goal": goal_pix_dev,
            "action": torch.zeros(1, 1, ACTION_DIM_FLAT, device=device),  # (B, T=1, 25)
        }
    ...
    def eval_grid_costs(model, obs_pix_dev, goal_pix_dev):
        ...
        for start in range(0, N, batch_size_grid):
            ...
            action_cands = bs_actions.unsqueeze(1)  # (bs, S=1, 5, 10)
    ...
    plot_specs = [
        ("Pretrained WM", Z_base, alphas_path_base, betas_path_base,
         models_data["cube.pretrained"]["markers"]),
        ("Adversarial WM", Z_adv, alphas_path_adv, betas_path_adv,
         models_data["cube.adv"]["markers"]),
    ]
    ...
    plot_path = testing_dir / "cube_landscape.png"
...
def main(
    ...
    print(
        f"Generating cube loss landscape "
    ...
    if ret.get("plot_bytes"):
        local_plot = Path("eval_results/cube_landscape.png")
    ...
    if ret.get("tar_bytes"):
        ...
        for model_name in ("cube.pretrained", "cube.adv"):
